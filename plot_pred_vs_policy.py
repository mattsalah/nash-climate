...
if dataset == 'cscc':
    outfile = f'output/charts/cscc_plot_{year}_spec{spec}.png'
    if exclude_countries:
        outfile = outfile.replace('.png', f'_no_{"_".join(exclude_countries)}')
    create_plot([f'noncoop_optimal_mu_spec{spec}'], f'Non-Cooperative Nash Equilbrium (CSCC) vs Counterfactual ({year}) - Spec {spec}', outfile, exclude_countries)
else:
    spec_map = {
    ...
    outfile = f'output/charts/rice_spec{spec}_plot_{year}.png'
    if exclude_countries:
        outfile = outfile.replace('.png', f'_no_{"_".join(exclude_countries)}')
    create_plot([f'NONCOOP_{scenario}'], f'Non-Cooperative Nash Equilbrium (RICE) vs Counterfactual ({year}) - Spec {spec}', outfile, exclude_countries)
